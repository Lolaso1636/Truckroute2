const PROXY_BASE = "http://localhost:3000"; // para geocodificación (nominatim)
const API_BASE = "http://localhost:4000";   // para tu backend (auth, truck, trips)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Capa para manejar los peajes (así se pueden limpiar fácilmente)
let peajesLayer = L.layerGroup().addTo(map);
...
async function mostrarPeajes(coords) {
  try {
    // Limpiar peajes anteriores antes de dibujar los nuevos
    peajesLayer.clearLayers();

    // Llamar al backend con las coordenadas de la ruta
    const res = await fetch('http://localhost:4000/api/peajes-along', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ coords, radius: 2000 }) // 2000 m de radio
    });
    const data = await res.json();
    if (!data.features) {
      console.warn('Sin peajes cercanos');
      return;
    }

const iconPeaje = L.icon({
  iconUrl: 'http://127.0.0.1:5500/frontend/images/peaje.png',
  iconSize: [32, 32],
  iconAnchor: [16, 32],
  popupAnchor: [0, -28]
});
    ...
    // Dibujar los peajes en el mapa
    data.features.forEach(f => {
      const [lng, lat] = f.geometry.coordinates;
      const p = f.properties || {};
      const nombre = p.nombre_peaje || p.nombre || 'Peaje';
      const dist = p.dist_to_start_m || 0;
      const sector = p.sector || '';

      const popupHtml = `
        <b>${nombre}</b><br>
        ${sector ? sector + '<br>' : ''}
        Distancia desde inicio: ${(dist / 1000).toFixed(1)} km
      `;

      const marker = L.marker([lat, lng], { icon: iconPeaje }).bindPopup(popupHtml);
      peajesLayer.addLayer(marker);
    });

    console.log(`✅ ${data.features.length} peajes encontrados`);
  } catch (err) {
    console.error('Error mostrando peajes:', err);
  }
}
  ...
  markOriginBtn && markOriginBtn.addEventListener("click", () => { 
    markMode = "origin"; 
    alert("Clic en el mapa para ORIGEN"); 
  });

  markDestBtn && markDestBtn.addEventListener("click", () => { 
    markMode = "dest"; 
    alert("Clic en el mapa para DESTINO"); 
  });
    ...
    if (coords.length > 0) {
      ...
      if (token) {
        try {
          ...
          console.warn('no se guardó trip en backend', e); 
        }
      }
    }
