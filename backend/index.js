...
// Cargar archivo GeoJSON de peajes
const peajesPath = path.join(__dirname, '../peajes.geojson');
let peajesData = null;

try {
  const data = fs.readFileSync(peajesPath, 'utf8');
  peajesData = JSON.parse(data);
  console.log('✅ Peajes cargados correctamente:', peajesData.features.length);
} catch (error) {
  console.error('❌ Error al cargar peajes.geojson:', error);
}
...
// Aumentar el límite del tamaño del body JSON
...
/**
 * POST /api/peajes-along
 * Body: { coords: [[lat, lon], [lat, lon], ...], radius: 100 }  // coords en el mismo formato que usas para dibujar (lat,lng)
 * Devuelve FeatureCollection con properties: { ...originalProps, dist_to_route_m, dist_to_start_m }
 */
app.post('/api/peajes-along', express.json(), (req, res) => {
  try {
    const coordsIn = req.body.coords;
    const radiusMeters = Number.isFinite(req.body.radius) ? Number(req.body.radius) : 100;

    if (!Array.isArray(coordsIn) || coordsIn.length < 2) {
      return res.status(400).json({ error: 'Se requieren coords (array de [lat,lon]) con al menos 2 puntos' });
    }

    // Convertir coords a [lng, lat] para turf
    const lineCoords = coordsIn.map(c => [Number(c[1]), Number(c[0])]);

    // 🔹 Simplificar ruta (reduce puntos en rutas largas sin perder forma)
    let line = turf.lineString(lineCoords);
    if (lineCoords.length > 500) {
      line = turf.simplify(line, { tolerance: 0.01, highQuality: false });
    }

    // 🔹 Cargar peajes
    const geoPath = path.join(__dirname, '..', 'peajes.geojson');
    if (!fs.existsSync(geoPath)) {
      return res.status(500).json({ error: 'peajes.geojson no encontrado en servidor' });
    }
    const peajesGeo = JSON.parse(fs.readFileSync(geoPath, 'utf8'));
    const startPt = turf.point([lineCoords[0][0], lineCoords[0][1]]);

    const matched = [];
    const bufferKm = radiusMeters / 1000;

    for (const feat of peajesGeo.features) {
      if (!feat.geometry?.coordinates) continue;

      const pt = turf.point(feat.geometry.coordinates);
      const distToLineKm = turf.pointToLineDistance(pt, line, { units: 'kilometers' });
      if (distToLineKm <= bufferKm) {
        const distToStartKm = turf.distance(startPt, pt, { units: 'kilometers' });

        const cloneProps = { ...(feat.properties || {}) };
        cloneProps.dist_to_route_m = Math.round(distToLineKm * 1000);
        cloneProps.dist_to_start_m = Math.round(distToStartKm * 1000);
    ...
    matched.sort((a, b) => (a.properties.dist_to_start_m || 0) - (b.properties.dist_to_start_m || 0));

    console.log(`✅ ${matched.length} peajes encontrados`);
    return res.json({
    ...
    console.error('❌ Error en /api/peajes-along:', e);
    return res.status(500).json({ error: 'Error interno al calcular peajes', detalle: e.message });
  }
});
